...
@app.route('/')
def health_check():
    return "COSMIC ZERO-SUM ENGINE ONLINE", 200
...
# --- 3. ZERO-SUM BALANCED PREDICTION ENGINE ---
class ZeroSumBridgeEngine:
    def __init__(self, history_scores):
        self.raw = np.array(history_scores, dtype=float)
        self.n = len(self.raw)
        # Quy đổi: Tài (>=11) thành +1, Xỉu (<=10) thành -1 (Zero-sum đối xứng tuyệt đối)
        self.centered = np.array([1 if x >= 11 else -1 for x in history_scores], dtype=float)

    def execute_pipeline(self):
        if self.n < 3:
            res = random.choice(["TÀI", "XỈU"])
            return {"result": res, "p_display": round(random.uniform(51.0, 58.0), 1)}

        # Model 1: Trung bình tâm (Trend Momentum)
        m1 = np.mean(self.centered)

        # Model 2: Độ dài bệt (Streak Force)
        last_val = self.centered[-1]
        streak = 0
        for val in reversed(self.centered):
            if val == last_val: streak += 1
            else: break
        m2 = (last_val * min(streak, 4)) / 4.0

        # Model 3: Markov Chain Bậc 2 đối xứng
        m3 = 0.0
        if self.n >= 4:
            state_map = {}
            for i in range(self.n - 2):
                key = (int(self.centered[i]), int(self.centered[i+1]))
                state_map.setdefault(key, []).append(self.centered[i+2])
            curr_key = (int(self.centered[-2]), int(self.centered[-1]))
            if curr_key in state_map and len(state_map[curr_key]) > 0:
                m3 = float(np.mean(state_map[curr_key]))

        # Tổng hợp Meta-Ensemble theo trọng số cân bằng
        total_score = (0.4 * m1) + (0.4 * m2) + (0.2 * m3)

        if abs(total_score) < 0.01:
            total_score = random.choice([-0.15, 0.15])

        result = "TÀI" if total_score > 0 else "XỈU"
        
        magnitude = min(abs(total_score), 1.0)
        p_display = round(50.0 + (magnitude * 42.0), 1)
        if p_display > 93.0: p_display = 93.0
        if p_display < 51.0: p_display = 51.0

        return {"result": result, "p_display": p_display}

class ZeroSumMD5Engine:
    def __init__(self, raw_hex):
        self.hex_str = raw_hex.lower().strip()

    def execute_pipeline(self):
        try:
            hash_int = int(self.hex_str, 16)
            direction = hash_int & 1
            ...
            deviation = abs(entropy - 2.0) / 2.0
            
            p_display = round(50.0 + (deviation * 38.0), 1)
            if p_display > 91.0: p_display = 91.0
            if p_display < 51.0: p_display = 51.0
...
if __name__ == '__main__':
    ...
    print("ZERO-SUM ENGINE BOT ONLINE...")
